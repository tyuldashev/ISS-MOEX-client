...
async def get_history_intervals(sec_id, board, market, engine):
    """
    Предполагается, что после выбора пользователем инструмента сайт сделает запрос get_security_attributes
    (или через pandas) и в каком-то виде
    запомнит полученные данные (возвращается словарь вида):
